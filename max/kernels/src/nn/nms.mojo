...
fn non_max_suppression[
    ...
    for b in range(batch_size):
        for c in range(num_classes):
            # Get pointer to scores for this batch and class
            # Entries will be set to MIN when they no longer correspond to eligible boxes.
            # This happens when:
            #   1. Score does not meet score threshold (filtered out initially)
            #   2. IoU with an already-selected box is above the IoU threshold (suppressed)
            var offset = scores.runtime_layout(
                RuntimeTuple[scores.layout.shape](b, c, 0)
            )
            var per_class_scores_ptr = scores.ptr.offset(offset)

            # Phase 1: Filter boxes by score threshold
            ...
            # Phase 2: Sort box indices by descending score
            # After sorting, box_idxs[0] points to the highest-scoring box
            sort[_greater_than](box_idxs)

            # Phase 3: Iteratively select boxes and suppress overlapping ones
            var pred_idx = 0
            while (
                ...
                # Output this box as a selected prediction
                # Each output contains 3 values: [batch_index, class_index, box_index]
                func(b, c, box_idxs[pred_idx])

                # At this point in the loop, box_idxs is sorted such that:
                # [selected boxes..., remaining candidates sorted by score..., suppressed boxes (score=MIN)...]
                var num_boxes_curr_pred = num_boxes_remaining

                # Phase 4: Suppress boxes with high IoU overlap
                ...
                # Optimization: Only sort the remaining unsorted portion of box_idxs
                # We don't need to sort the entire array because:
                #   1. The start contains already-selected boxes (order doesn't matter)
                #   2. The end contains suppressed boxes with score=MIN (order doesn't matter)
