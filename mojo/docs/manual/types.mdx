...
* Iterating a `List` returns a
  [reference](/mojo/manual/values/lifetimes#working-with-references) to each
  item, which is copied into name you provide by default:
...
Mojo warns you that the assignment to `item` isn't doing anything useful,
because `item` is a copy of value from the list, and it's overwritten on the
next loop iteration.
