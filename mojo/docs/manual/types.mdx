...
* Iterating a `List` returns a reference to each item, which is copied into
...
Mojo warns you that the assignment to `i` isn't doing anything useful.
...
The `Dict` iterators all yield references, which are copied into the declared
name by default, but you can use the `ref` marker to avoid the copy:
